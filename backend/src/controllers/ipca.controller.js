...
async function salvar(req, res, next) {
  try {
    ...
    await db.query(
      `INSERT INTO ipca_historico (mes_ano, valor)
       VALUES ($1, $2)
...
async function remover(req, res, next) {
  try {
    ...
    await db.query('DELETE FROM ipca_historico WHERE mes_ano = $1', [mesAno + '-01']);
    res.json({ ok: true });
  } catch (e) { next(e); }
}

/**
 * Sincroniza IPCA com BACEN (série 433).
 * Últimos 10 anos de dados mensais.
 */
